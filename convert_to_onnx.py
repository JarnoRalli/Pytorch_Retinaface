...
def main():
    ...
    parser.add_argument("-b", "--batch_size", default=1, help="Network batch size")
    parser.add_argument(
    ...
    # net and model
    net = RetinaFace(
        cfg=cfg, phase="test-decode", image_size=(args.long_side, args.long_side)
    )
    ...
    inputs = torch.randn(args.batch_size, 3, args.long_side, args.long_side).to(device)
    ...
    torch_out = torch.onnx.export(
        ...
        verbose=False,
        input_names=["input0"],
        output_names=["bboxes", "classes", "landmarks"],
        dynamic_axes={
            "input0": {0: "batch_size"},
            "bboxes": {0: "batch_size"},
